...
namespace Project_Cashier
{
    public partial class SellingForm : Form
    {
        ...
        private void populate()
        {
            ...
            string query = "select ProdName, ProdQty, ProdPrice from ProductTable";
        ...
        private void ProdDGV1_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            ProdPrice.Text = ProdDGV1.SelectedRows[0].Cells[2].Value.ToString();
        }
        ...
        {  
            ...
            else
            {
                ...
                {
                    Con.Open();
                    string query = "select ProdQty from ProductTable where ProdName = '" + ProdName.Text + "'";
                    SqlDataAdapter sda = new SqlDataAdapter(query, Con);
                    SqlCommandBuilder builder = new SqlCommandBuilder(sda);
                    var dt = new DataTable();
                    sda.Fill(dt);
                    quantity = (Int32)dt.Rows[0][0];
                    Con.Close();
                }catch(Exception ex)
                {
                ...
                if (quantity >= int.Parse(ProdQty.Text))
                {
                    ...
                    Con.Open();
                    string query = "update ProductTable set ProdQty = '" + (quantity - int.Parse(ProdQty.Text)).ToString() + "'where ProdName = '" + ProdName.Text + "'";
                ...
                {
                    MessageBox.Show("we have only " + quantity.ToString() + " from this Product!");
                }
        ...
        {
            Con.Open();
            string query = "select ProdName, ProdQty, ProdPrice from ProductTable where ProdCat ='" + comboBox2.SelectedValue.ToString() +"'";
            SqlDataAdapter sda = new SqlDataAdapter(query, Con);
            SqlCommandBuilder builder = new SqlCommandBuilder(sda);
            var ds = new DataSet();
            sda.Fill(ds);
